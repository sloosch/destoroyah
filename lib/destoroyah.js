...
var AnyOfAttack, Attack, BoolAttack, CharAttack, DecimalAttack, IntAttack, NDecimalAttack, NIntAttack, ObjectAttack, PDecimalAttack, PIntAttack, PileOfAttack, SignAttack, StringAttack, attackRegistry, constants, field, registerAttack,
  __slice = [].slice,
  ...
  ObjectAttack.prototype._perform = function(dist, example) {
    ...
    for (k in example) {
      ...
      obj[k] = (function() {
        switch (typeof v) {
          case 'boolean':
            return attack.bool().execute();
          case 'number':
            return attack.decimal().execute(dist);
          case 'string':
            return attack.string().execute(dist);
          case 'object':
            return attack.object(v).execute(dist);
        }
      })();
    }
    return obj;
...
var defaultHopes, f, hopeName, hopeRegistry, registerHope,
  ...
  Rampage.prototype.attacks = function() {
    ...
    return attackNames.map((function(_this) {
      return function(genName) {
        ...
        if (attackName in destoroyah.attack) {
          return destoroyah.attack[attackName]();
        }
        if (attackName in ((_ref = _this.monster) != null ? _ref.additionalAttack : void 0)) {
          return _this.monster.additionalAttack[attackName]();
        }
